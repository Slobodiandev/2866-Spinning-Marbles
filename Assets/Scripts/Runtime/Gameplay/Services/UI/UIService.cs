...
namespace Runtime.Gameplay.Services.UI
{
    public sealed class UiService : IUiService
    {
        ...
        {
            GameObject container = await _assetProvider.InstantiateGameObject(ConstScreens.UiServiceViewContainer);
            _uiServiceViewContainer = container.GetComponent<UiServiceViewContainer>();

            RegisterScreenPrototypes();

            RegisterWindowPrototypes();
        }

        private void RegisterWindowPrototypes()
        {
        ...
        private void RegisterScreenPrototypes()
        {
        ...
        public bool IsScreenActive(string id)
        {
        ...
        public async UniTask<BaseWindow> ShowWindow(string id, BaseWindowData data = null, CancellationToken cancellationToken = default)
        {
            if (_popupPrototypes.TryGetValue(id, out GameObject prototype))
            {
                var popup = _factory.Create<BaseWindow>(prototype, _uiServiceViewContainer.ScreenParent);
                await popup.ShowWindow(data, cancellationToken);
                return popup;
            }
        ...
        public T GetWindow<T>(string id) where T : BaseWindow
        {
            if (_popupPrototypes.TryGetValue(id, out GameObject prototype))
            {
                var popup = _factory.Create<T>(prototype, _uiServiceViewContainer.ScreenParent);
                popup.HideWindowImmediately();
                return popup;
            }
